...
if isempty(bank_num)
    bank_num = ceil(4.6 * log10(fs));               % default number of filters
end

if bank_num < 1
    bank_num = round(melrng / (bank_num * 1000)) - 1;
end
...
centre_freq = mflh(1) + (1 : p) * melinc;           % mel centre frequencies
...
pf = ( freq_to_mel( (b1 : b4) * fs / n ) - mflh(1) ) / melinc;
